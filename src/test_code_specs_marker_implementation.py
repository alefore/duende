...
# ✨
  ...
  async def test_init_valid_python_marker(self) -> None:
    ...
    value = '''# ✨ test init valid python marker
    a = 1 + 1
    # ✨'''
...
# ✨
  ...
  async def test_init_valid_javascript_marker(self) -> None:
    ...
    value = '''// ✨ test init valid javascript marker
    let b = 2 + 2;
    // ✨'''
    expected_comment_char = "//"
    marker = MarkerImplementation(name, value, expected_comment_char)
...
# ✨
  ...
  async def test_init_single_line_valid_marker(self) -> None:
    ...
    value = '''# ✨ test init single line valid marker
    pass
    # ✨'''
    expected_comment_char = "#"
    marker = MarkerImplementation(name, value, expected_comment_char)
...
# ✨
  ...
  async def test_init_incorrect_start_marker_name_raises_error(self) -> None:
    ...
    with self.assertRaisesRegex(
        ValueError,
        r"Implementation value must start with a line matching '# ✨ correct marker name'. Got: '# ✨ incorrect marker name'"
    ):
      MarkerImplementation(name, value, expected_comment_char)
# ✨
  ...
  async def test_init_no_start_marker_raises_error(self) -> None:
    ...
    with self.assertRaisesRegex(
        ValueError,
        r"Implementation value must start with a line matching '# ✨ test init no start marker raises error'. Got: '# This line does not contain the expected start marker'"
    ):
      MarkerImplementation(name, value, expected_comment_char)
# ✨
  ...
  async def test_init_no_end_marker_raises_error(self) -> None:
    ...
    value = '''# ✨ test init no end marker raises error
    some_test_code = 1
    # This line does not contain the expected end marker'''
    expected_comment_char = "#"
    with self.assertRaisesRegex(
        ValueError,
        r"Implementation value must end with a line matching '# ✨'. Got: '    # This line does not contain the expected end marker'"
    ):
      MarkerImplementation(name, value, expected_comment_char)
# ✨
...
# Some footer"""
    ...
    implementation_value = '''# ✨ test save replaces single marker python file
...
// Some footer"""
    ...
    implementation_value = '''// ✨ test save replaces single marker javascript file
...
# Some footer"""
    ...
    implementation_value = '''# ✨ test save marker not found raises error
...
# Some footer"""
    ...
    implementation_value = '''# ✨ test save multiple markers raises error
    ...
    with self.assertRaisesRegex(
        ValueError,
        f"Marker '{{{{🍄 {marker_name}}}}}' found 2 times in '.*'. Expected exactly one."
    ):
      await marker_implementation.save(file_path)
# ✨
...
# Some footer"""
    ...
    implementation_value = '''# ✨ test save implementation with leading and trailing whitespace
